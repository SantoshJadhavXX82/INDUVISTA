...
export default function TagPicker({ selectedIds, onChange, maxTags = 6 }: TagPickerProps) {
  ...
  // Fetch every tag once (enabled and disabled). Disabled tags are still
  // shown — historians do log even disabled tags transiently, and operators
  // want to know what exists. Disabled tags get a DISABLED badge so they
  // can't be confused with active ones. Use the API max (2000) so even
  // large-site deployments see everything; client-side filter handles
  // search. If a deployment outgrows 2000 tags we'll need server-side
  // search via the `q` query parameter.
  ...
  const matched = ql
    ...
    : tags;
  // Stable sort: enabled tags first, then by name. Operators almost always
  // want logging-enabled tags; surfacing disabled tags below keeps them
  // discoverable without burying the useful ones.
  const filtered = [...matched].sort((a, b) => {
    const ea = a.logging_enabled ? 0 : 1;
    const eb = b.logging_enabled ? 0 : 1;
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate flex items-center gap-1.5">
                        ...
                        {!t.logging_enabled && (
                          <span className="inline-flex items-center rounded-sm bg-amber-100 px-1.5 py-0 text-[9px] font-semibold uppercase tracking-wider text-amber-800">
                            disabled
                          </span>
                        )}
                      </div>
