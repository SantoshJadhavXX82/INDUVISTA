...
type TrendChartProps = {
  ...
  /** Set of tag IDs the operator has chosen to hide. Synced into uPlot
   *  via setSeries(idx, { show }). Quality-marker series for the same
   *  tag are hidden in lockstep. */
  hiddenTagIds?: Set<number>;
  /** Spec 9.4 - filter samples by quality before plotting. "good_only"
   *  also hides the bad/uncertain marker overlays so the chart shows an
   *  honest known-good view. */
  qualityFilter?: "all" | "hide_bad" | "good_only";
  /** Spec 16.1 Option A - draw a translucent min/max envelope band
   *  around each aggregated series. No-op for raw mode (no buckets). */
  showEnvelope?: boolean;
};

const TrendChart = forwardRef<TrendChartHandle, TrendChartProps>(
  function TrendChart({
    history, height = 420, tooltipMode = "full",
    hiddenTagIds, qualityFilter = "all", showEnvelope = false,
  }, ref) {
    ...
    // Sync the operator's show/hide selections (from Live Value Panel
    // tiles) into uPlot. Both the main line and its quality-marker
    // siblings carry __tagId, so they hide/show in lockstep.
    useEffect(() => {
      const plot = plotRef.current;
      if (!plot) return;
      plot.series.forEach((s, idx) => {
        if (idx === 0) return;                 // X axis
        const tagId = (s as unknown as { __tagId?: number }).__tagId;
        if (tagId == null) return;
        const shouldShow = !(hiddenTagIds?.has(tagId) ?? false);
        if (shouldShow !== (s.show ?? true)) {
          plot.setSeries(idx, { show: shouldShow });
        }
      });
    }, [hiddenTagIds, history]);
    ...
    useEffect(() => {
      ...
      const tryMount = () => {
        ...
        try {
          const { data, opts } = buildChart(history, width, height, is24h, qualityFilter, showEnvelope);
    ...
    }, [history, height, is24h, qualityFilter, showEnvelope]);
...
function buildChart(
  ...
  height: number,
  is24h: boolean,
  qualityFilter: "all" | "hide_bad" | "good_only" = "all",
  ...
  // The quality filter (spec 9.4) is applied here: samples that don't pass
  // the filter become null in the Y array. spanGaps=false on the line
  // series means filtered-out segments visibly break, signaling to the
  // operator that data was withheld rather than missing.
  const ys: (number | null)[][] = history.series.map((s) => {
    const lookup = new Map<number, number | null>();
    s.points.forEach((p) => {
      const t = new Date(p.t).getTime();
      if (isNaN(t)) return;
      const passes = qualityFilter === "all"
        ? true
        : passesQualityFilter(p, isRaw, qualityFilter);
      lookup.set(Math.floor(t / 1000), passes ? (p.v ?? null) : null);
    });
  ...
  const series: uPlot.Series[] = [
    ...
    ...history.series.map((s, idx) => {
      ...
      return {
        ...
        // Custom metadata for show/hide sync from Live Value Panel.
        // uPlot ignores unknown props but they round-trip on plot.series.
        __tagId: s.tag_id,
        stroke: color,
        width: 1.6,
        spanGaps: false,
  ...
  history.series.forEach((s, idx) => {
    ...
    try {
      // When the quality filter hides bad/uncertain samples from the
      // line, the matching marker overlays are also dropped so the chart
      // doesn't contradict the filter (e.g. "good only" with bad dots).
      const allowBadMarkers       = qualityFilter === "all";
      const allowUncertainMarkers = qualityFilter !== "good_only";

      const badData = allowBadMarkers
        ? buildQualityMarkerData(s.points, xs, "bad", isRaw)
        : xs.map(() => null);
      const badCount = badData.filter((v) => v != null).length;
      const uncertainData = allowUncertainMarkers
        ? buildQualityMarkerData(s.points, xs, "uncertain", isRaw)
        : xs.map(() => null);
  ...
  // ---- 5b. Min/Max Envelope (spec 16.1 Option A) ------------------------
  // For each aggregated tag, derive two helper series carrying the bucket
  // min and max. uPlot will draw a translucent fill BETWEEN them via the
  // opts.bands config below. The series themselves are invisible
  // (transparent stroke, width=0) - they only exist to anchor the band.
  //
  // Order matters: envelope series get pushed AFTER markers so they sit at
  // the tail of the series array. Each tag with envelope data contributes
  // exactly two consecutive series (max first, then min).
  type EnvelopeTrack = {
    tagId: number;
    color: string;
    maxData: (number | null)[];
  ...
  const envelopeTracks: EnvelopeTrack[] = [];
  if (showEnvelope && !isRaw) {
    history.series.forEach((s, idx) => {
      const color = TAG_COLORS[idx % TAG_COLORS.length];
      const scale = scaleAssignment[idx];
      const maxLookup = new Map<number, number>();
      const minLookup = new Map<number, number>();
      s.points.forEach((p) => {
        const t = new Date(p.t).getTime();
        if (isNaN(t)) return;
        const key = Math.floor(t / 1000);
        if (p.mx != null) maxLookup.set(key, p.mx);
        if (p.mn != null) minLookup.set(key, p.mn);
      });
      const maxData = xs.map((t) => maxLookup.get(t) ?? null);
      const minData = xs.map((t) => minLookup.get(t) ?? null);
      const hasMx = maxData.some((v) => v != null);
      const hasMn = minData.some((v) => v != null);
      if (!hasMx && !hasMn) return;

      envelopeTracks.push({ tagId: s.tag_id, color, maxData, minData });
      // Push both helper series. class=u-marker-row keeps them out of
      // any legend; __tagId lets the show/hide sync hide them with the
      // parent tag.
      series.push({
        label: `${s.tag_name} · env-max`,
        stroke: "transparent",
        width: 0,
        spanGaps: false,
        scale,
  ...
  // Bands tying each tag's max + min envelope helper series. Indices are
  // computed from the known order: X (0) + main lines + markers + envelopes.
  // For each EnvelopeTrack at i, max series is at envBaseIdx + i*2 and
  // min is at envBaseIdx + i*2 + 1.
  const envBaseIdx = 1 + ys.length + markerSeries.length;
  const bands: NonNullable<uPlot.Options["bands"]> = envelopeTracks.map((e, i) => ({
    series: [envBaseIdx + i * 2, envBaseIdx + i * 2 + 1] as [number, number],
    // 8-digit hex: ~20% alpha. Strong enough to read against the chart
    // background, soft enough not to compete with the main line color.
    fill: `${e.color}33`,
  }));
  ...
  const opts: uPlot.Options = {
    ...
    // uPlot's bottom-of-chart legend is intentionally disabled. The Live
    // Value Panel above the chart already shows color, name, EU, current
    // value and quality per tag - in a layout we control - so the inline
    // legend was duplicating it and causing long-name overflow into
    // neighboring cells. Cursor values are available on hover via the
    // rich tooltip (and stay visible when click-pinned).
    legend: { show: false },
    hooks: {
      draw: [
...
/**
 * Decide whether a sample passes the operator's quality filter (spec 9.4).
 *
 * Raw mode: classified by point.st (192-255 good, 128-191 good,
 *   64-127 uncertain, 0-63 bad). Unknown ST (null) passes through
 *   "hide_bad" but is filtered out by "good_only".
 *
 * Aggregated mode: classified by bucket counts. A bucket is considered
 *   "bad" if any bad samples landed in it, "uncertain" if any uncertain
 *   landed and no bad, otherwise "good". This is the same rule the
 *   tooltip uses, so chart line and tooltip stay consistent.
 */
function passesQualityFilter(
  p: TrendPoint,
  isRaw: boolean,
  filter: "all" | "hide_bad" | "good_only",
): boolean {
  if (filter === "all") return true;
  if (isRaw) {
    if (p.st == null) return filter !== "good_only";
    if (filter === "hide_bad")  return p.st >= 64;
    if (filter === "good_only") return p.st >= 128;
    return true;
  }
  // Aggregated
  const bad = p.b ?? 0;
  const good = p.g ?? 0;
  const total = good + bad; // uncertain count not exposed yet
  if (filter === "hide_bad")  return bad === 0;
  if (filter === "good_only") return bad === 0 && good === total;
  return true;
}
