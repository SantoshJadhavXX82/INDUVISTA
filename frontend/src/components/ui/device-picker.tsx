...
export function DevicePicker({
  ...
  const rows: Row[] = useMemo(() => {
    // Phase 11 — only show "recent" labels when there's a meaningful gap
    // between total devices and the recent set. With 4 devices and 3
    // recents, tagging 3 of 4 looks arbitrary; the label only helps when
    // recents are actually surfacing a small subset from a larger pool.
    // Rule: total must exceed (MAX_RECENTS + 1) for labels to appear.
    const showRecentLabels = devices.length > MAX_RECENTS + 1;
    const recentSet = new Set(recents);
    ...
    for (const d of filteredDevices) {
      list.push({
        kind: "device",
        device: d,
        isRecent: showRecentLabels && query.trim() === "" && recentSet.has(d.id),
      });
    }
    return list;
  }, [filteredDevices, recents, query, devices.length]);
