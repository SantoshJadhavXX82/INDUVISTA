 ...
 *   - Phase 13.12 — Rate-of-change line (least-squares slope over the
 *     rolling 5-min buffer maintained per tag here)
 *
 * Polling cadence:
...
// Phase 13.12 — rolling ROC buffer caps. 30 samples × 2-s poll = 60 s,
// 30 samples × 10-s poll = 5 min. Either way we're inside the 5-min
// window that trendRoc.computeROC respects on its own.
const ROC_BUFFER_CAP = 30;
const ROC_BUFFER_AGE_MS = 5 * 60 * 1000;
...
export default function LiveValuePanel({
  ...
  // has arrived. The same tick also re-reads the ROC unit from
  // localStorage so toggles in TrendSummaryPanel propagate here within
  // ~1 s without prop drilling.
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setTick((v) => v + 1);
      const current = loadRocUnit();
      setRocUnit((prev) => (prev !== current ? current : prev));
    }, 1000);
  ...
  // Phase 13.12 — rolling sample buffer per tag for ROC computation.
  // Pushed on each fetch when the timestamp advances; trimmed by both
  // count and age. Persisted only in memory — clears on reload, which
  // is correct (the buffer reflects "what we've observed this session").
  const rocBuffersRef = useRef<Map<number, RocSample[]>>(new Map());
  ...
  // Diff against previous fetch + push to ROC buffer.
  tags.forEach((t) => {
    ...
    // Push to ROC buffer if we have a fresh timestamp + numeric value.
    // Duplicate timestamps (poll returning the same sample twice) are
    // skipped so the least-squares fit doesn't see a zero-Δt point.
    if (t.current_value_double != null && t.last_update_utc) {
      const tMs = new Date(t.last_update_utc).getTime();
      if (isFinite(tMs)) {
        const buf = rocBuffersRef.current.get(t.id) ?? [];
        const last = buf[buf.length - 1];
        if (!last || last.t !== tMs) {
          buf.push({
            t: tMs,
            v: t.current_value_double,
            q: t.current_st ?? undefined,
          });
        }
        // Trim: keep last ROC_BUFFER_CAP and drop anything older than
        // ROC_BUFFER_AGE_MS. Compose into one pass for clarity.
        const cutoff = Date.now() - ROC_BUFFER_AGE_MS;
        const trimmed = buf
          .filter((s) => s.t >= cutoff)
          .slice(-ROC_BUFFER_CAP);
        rocBuffersRef.current.set(t.id, trimmed);
      }
    }
  });
        ...
        >
          {tags.map((t, idx) => (
            <Tile
              ...
              rocSamples={rocBuffersRef.current.get(t.id) ?? []}
              rocUnit={rocUnit}
            />
          ))}
        </div>
...
function Tile({
  tag, color, change, isHidden, onToggleHidden, rocSamples, rocUnit,
}: {
  tag: TrendTag;
  color: string;
  ...
  // Phase 13.12 — ROC from the rolling buffer.
  const roc = useMemo(
  ...
  return (
    ...
    >
      ...
      {/* Row 3: ROC (Phase 13.12) */}
      <div
        className="text-[10px] tabular-nums"
        data-live-roc={tag.id}
        title={rocTooltip(roc)}
      >
        <span className="text-muted-foreground">ROC: </span>
        <span className="font-medium text-foreground">
          {formatROC(roc, tag.engineering_unit)}
        </span>
      </div>

      {/* Row 4: quality + ST + age */}
