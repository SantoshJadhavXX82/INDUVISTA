...
import {
  EVALUABLE_RULE_TYPES, RULE_TYPE_LABELS, SEVERITY_LABELS,
} from "@/types/alarms";
              ...
              <tbody>
                {rules.map((r) => (
                  <tr key={r.id} className="border-t border-border hover:bg-secondary/30">
                    ...
                    <td className="px-3 py-1.5">
                      <Badge variant="outline" className={`text-[10px] ${severityBadgeClass(r.severity)}`}>
                        {SEVERITY_LABELS[r.severity]}
                      </Badge>
                    </td>
                    <td className="px-3 py-1.5 text-right tabular-nums">{r.threshold}</td>
...
function RuleForm({
  form, onChange, error, saving, tagOptions, tagsLoading,
  onSubmit, onCancel,
}: {
  form: FormState;
  ...
  return (
    <div className="border-b border-border bg-secondary/20 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        {/* Deadband */}
        <label className="flex flex-col gap-1 text-xs">
          <span className="text-muted-foreground" title="Hysteresis around the threshold to prevent chatter">
            Deadband
          </span>
