...
export const BLOCK_SCHEMA_OVERRIDES: Record<string, Record<string, FieldOverride>> = {
  ...
  COUNT_GOOD:      { inputs: { label: "Inputs to scan for good quality" } },
  COUNT_NONZERO:   { inputs: { label: "Inputs to scan for nonzero",   allow_constant: true } },
  FIRST_GOOD:      { inputs: { label: "Candidates (use first good)" } },
  LAST_GOOD:       { inputs: { label: "Candidates (use last good)" } },
  HIGHEST_QUALITY: { inputs: { label: "Candidates (use best quality)" } },
  WEIGHTED_AVG: {
    values:  { label: "Values" },
    weights: { label: "Weights", allow_constant: true },
    inputs:  { label: "Values & weights" },
  },
  ...
  HOT_STANDBY: {
    primary: { label: "Primary (use when good)" },
    backup:  { label: "Backup (use when primary is bad)" },
  },
  VOTING_M_OF_N: {
  ...
  // ===== Edge detectors =====
  R_TRIG: { input: { label: "Signal (one-shot on rising edge)" } },
  F_TRIG: { input: { label: "Signal (one-shot on falling edge)" } },

  // ===== Latches =====
  SR: {
    set:   { label: "Set (S has priority)" },
    reset: { label: "Reset" },
    s:     { label: "Set (S has priority)" },
    r:     { label: "Reset" },
  },
  RS: {
    set:   { label: "Set" },
    reset: { label: "Reset (R has priority)" },
    s:     { label: "Set" },
    r:     { label: "Reset (R has priority)" },
  },

  // ===== Counters =====
  CTU: {
    count_input:  { label: "Count input (counts on rising edge)" },
    reset_input:  { label: "Reset to 0" },
    cu:           { label: "Count input (counts on rising edge)" },
    reset:        { label: "Reset to 0" },
    preset:       { label: "Preset count (output goes ON at this value)" },
    preset_value: { label: "Preset count (output goes ON at this value)" },
  },
  CTD: {
    count_input:  { label: "Count input (counts down on rising edge)" },
    load_input:   { label: "Load preset value into counter" },
    cd:           { label: "Count input (counts down on rising edge)" },
    ld:           { label: "Load preset value into counter" },
