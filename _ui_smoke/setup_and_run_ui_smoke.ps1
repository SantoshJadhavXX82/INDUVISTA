...
try {
    ...
    if ($exitCode -eq 0) {
        ...
        Write-Host "[FAIL] $exitCode test(s) failed - see playwright-report/index.html" -ForegroundColor Red
        Write-Host "       Open with:  npx playwright show-report" -ForegroundColor Yellow
    }
    exit $exitCode
}
finally {
