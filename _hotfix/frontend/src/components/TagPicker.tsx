...
export default function TagPicker({ selectedIds, onChange, maxTags = 6 }: TagPickerProps) {
  ...
  // Fetch every enabled tag once. 500-tag cap on the endpoint makes this
  // trivial; client-side filter handles search.
  const tagsQuery = useQuery({
    queryKey: ["trend-tags"],
    queryFn: () => api.get<TrendTag[]>("/trends/tags?enabled_only=true&limit=500"),
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {filtered.length > 100 && (
                <p className="p-2 text-[10px] text-muted-foreground text-center">
                  Showing first 100 of {filtered.length} matches — refine your search.
                </p>
              )}
            </div>
