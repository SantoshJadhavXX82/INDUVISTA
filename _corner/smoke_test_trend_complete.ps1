...
# Raw aggregation: only sensible over short windows. UI auto-routes longer
# windows to 1m/1h/1d. Use 30 minutes to avoid the 100K-row window-function
# slowness that the prior test version was timing out on.
$rRaw = Fetch-Json "/api/trends/history?tag_ids=$idsCsv&start=$(Iso-UtcMinusMinutes 30)&end=$(Iso-UtcNow)&aggregation=raw&max_points=2000" 60
if ($rRaw) {
    if ($rRaw.aggregation -eq 'raw') {
        Pass "aggregation=raw requested over 30m -> returned raw"
    } else {
        Fail "aggregation=raw requested -> backend returned $($rRaw.aggregation)"
    }
}

# 1d aggregation: needs a window where the 1d CA actually has rows.
# 30-day window avoids realtime-aggregate fallback to raw scanning.
$r1d = Fetch-Json "/api/trends/history?tag_ids=$idsCsv&start=$((Get-Date).AddDays(-30).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ'))&end=$(Iso-UtcNow)&aggregation=1d&max_points=2000" 90
if ($r1d) {
    if ($r1d.aggregation -eq '1d') {
        Pass "aggregation=1d requested over 30d -> returned 1d"
    } else {
        Warn "aggregation=1d requested -> backend returned $($r1d.aggregation) (1d CA may not have backfilled yet)"
    }
}
...
# Re-use $arr from Section A instead of re-fetching - the prior fetch
# was occasionally racing the DB and leaving us with empty data, which
# in turn made Section G silent and Section I emit a misleading "fewer
# than 10 enabled tags" WARN.
$enabledIds = @($enabledNumeric | ForEach-Object { $_.id })
$samples = @($arr | Where-Object { $enabledIds -contains $_.id })
if ($samples.Count -gt 0) {
    ...
    Fail "no sample tiles available (selected ids not in arr)"
}
...
$bigPool = @($arr | Where-Object { $_.logging_enabled -eq $true } | Select-Object -First 20)
