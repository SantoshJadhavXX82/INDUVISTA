...
export default function TrendTooltip({
  ...
  // Fixed top-right corner of the chart container. We used to follow the
  // cursor with offset, but that obscured exactly the datum the operator
  // was inspecting. Industrial-tool convention is to dock the tooltip in
  // a corner of the chart so the data area is never blocked. Cursor
  // position is still reflected in the tooltip's CONTENT (timestamp +
  // per-series values) - just not its location.
  const PAD = 8;
  const left = Math.max(PAD, containerWidth - TOOLTIP_WIDTH - PAD);
  const top = PAD;
  void containerHeight;  // no longer used for positioning; kept in props
  ...
  return (
    ...
    >
      <div className={`bg-card/95 backdrop-blur-sm border ${isPinned ? "border-blue-400 ring-1 ring-blue-200" : "border-border"} rounded-md shadow-lg overflow-hidden text-xs`}>
