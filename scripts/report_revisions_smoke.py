...
Throwaway, self-cleaning. Proves:
  ...
  * a revision whose validation FAILS (invalid custom period) cannot be activated (409)
...
def main() -> int:
    ...
    try:
        ...
        # validation gate: invalid custom period -> draft -> activate must 409
        ...
        (ok if r3.get("validation", {}).get("overall") == "failed" else bad)(
            "draft with invalid custom period validates failed", f"overall={r3.get('validation', {}).get('overall')}")
        try:
