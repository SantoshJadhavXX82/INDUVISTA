...
export default function TrendTooltip({
  cursor, tagsMeta, isAggregated, mode = "full", containerWidth, containerHeight,
}: TrendTooltipProps) {
  ...
  return (
    ...
    >
      <div className="bg-card border border-border rounded-md shadow-lg overflow-hidden text-xs">
        ...
        {/* Per-series rows
            pointer-events-auto here re-enables wheel/touch scrolling on
            the scrollable list. The outer wrapper stays pointer-events-none
            so the chart cursor can keep moving when the mouse is over the
            tooltip's edges; only the scroll area intercepts. */}
        <div
          className="overflow-y-auto pointer-events-auto"
          style={{ maxHeight: TOOLTIP_MAX_HEIGHT - 60 }}
        >
          {cursor.series.map((s) => {
